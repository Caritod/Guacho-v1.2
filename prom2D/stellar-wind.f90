...
subroutine init_wind()
  ...
  TSW   = 1.5E6     !*************   ! Stellar temperature (K)
  RSW   = 1.2*rsun   !*************
  VSW   = 0.e5       !*************      ! Stellar wind velocity (cm/s)
  dsw   = 1.5e-16!((AMDOT/RSW)/(4*pi*RSW*VSW))   ! Stellar density @RS (g cm^-3)
      ...
      do k=nzmin,nzmax
        ...
        if( rads <= rsw) then
          ...
          if (pmhd .or. mhd) then
            ...
            cv_gbl=cvar(u(neqdyn+1,i,j,k)/u(1,i,j,k))
