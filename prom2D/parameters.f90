...
module parameters
  ...
  !> Use a variable Gamma as a function of n_HI
  !> if true the value of cv is computed with n_HI
  logical, parameter :: cv_var = .false.
  ...
  integer, parameter :: nxtot=1      !< Total grid size in X
  ...
  !  set box size   
  real, parameter :: xmax=10*Rsun/nxtot          !< grid extent in X (code units)
  real, parameter :: ymax=1.          !< grid extent in Y (code units)
  real, parameter :: zmax=1.          !< grid extent in Z (code units)
  real, parameter :: xphys=10*Rsun    !< grid extent in X (physical units, cgs)
  ...
  integer            :: itprint0=2  !< number of output to do warm start
...
#endif
 integer, parameter :: ndim=2         !< num. of dimensions
